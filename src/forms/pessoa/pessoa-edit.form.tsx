...
import { TabDadosPessoais } from '@/forms/pessoa/tabs/tab-dados-pessoais'
import { TabDocumentos } from '@/forms/pessoa/tabs/tab-documentos'
import { TabContatos } from '@/forms/pessoa/tabs/tab-contatos'
import { TabDependentes } from '@/forms/pessoa/tabs/tab-dependentes'
import { forwardRef, useImperativeHandle } from 'react'
import { useSexo } from '@/hooks/common/sexo.hook'

type PessoaEditFormProps = {
  id: string
}

type PessoaEditFormRef = {
  submit: () => void
}

const PessoaEditForm = forwardRef<PessoaEditFormRef, PessoaEditFormProps>(
  ({ id }, ref) => {
    const { sexoOptions } = useSexo()

    const pessoa = pessoasMock.find((item) => item.id === id)
          ...
          <Card>
            <CardContent>
              ...
              <Tabs defaultValue="dependentes" className="mt-4 w-full">
                <TabsList>
                  <TabsTrigger value="dadosPessoais">
                    Dados Pessoais
                  </TabsTrigger>
...
PessoaEditForm.displayName = 'PessoaEditForm'
